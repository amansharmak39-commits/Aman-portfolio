...
export default function Projects() {
  ...
  const handleStep = (direction: 'left' | 'right') => {
    // Calculate step size based on project count (aiming to move roughly one card width)
    const stepSize = (1 / projects.length) * 50;
    const currentX = x.get();
    let targetX = direction === 'left' ? currentX + stepSize : currentX - stepSize;

    // Wrap around logic for infinite loop (0 to -50)
    // If we exceed 0, we jump to -50 + offset
    // If we exceed -50, we jump to 0 - offset
    ...
    animate(x, targetX, {
      ...
      mass: 0.5,
      onComplete: () => {
        // Normalize x after spring animation to keep it within [0, -50]
        const finalX = x.get();
        if (finalX > 0) x.set(finalX - 50);
        if (finalX < -50) x.set(finalX + 50);
      }
    });
  };
